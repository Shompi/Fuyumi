...
module.exports = async (message = new Message()) => {
  try {
    ...
    let blacklist = '+-loli+-rape+-scat'; //Tags blacklist

    let data = await fetch(endpoint + query + blacklist);
    if (!data.ok) return await message.channel.send("Error al conectar con el servidor, codigo: " + data.statusCode).catch(err => console.log(err));
