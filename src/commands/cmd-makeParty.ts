// @ts-check
import { EmbedBuilder, User, Collection, ButtonBuilder, ActionRowBuilder, Colors, ButtonStyle, Snowflake, ComponentType } from "discord.js";
import { Command } from "@sapphire/framework";

const timeouts: Collection<string, { t_expires: number, user: User }> = new Collection()

export class MakeParty extends Command {
	...
	public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		if (interaction.inCachedGuild()) {
			...
			const partyMembers: Collection<Snowflake, User> = new Collection();
			...
			const componentCollector = partyMessage.createMessageComponentCollector({ time: waitTime * 1000 * 60, componentType: ComponentType.Button });
				...
				.on('end', async () => {
					...
					try {
						if (spotsLeft > 0) {
							...
							const partyFail = new EmbedBuilder()
							...
							const partySuccessful = new EmbedBuilder()
...
function createActionRow() {
	...
	return new ActionRowBuilder<ButtonBuilder>().addComponents([JoinButton, LeaveButton, CancelButton]);
}